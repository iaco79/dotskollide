...
uniform vec4  uLightPosition; //IACO    [MAX_LIGHTS];
uniform vec4  uLightAmbient; //IACO     [MAX_LIGHTS];
uniform vec4  uLightDiffuse; //IACO     [MAX_LIGHTS];
#ifdef USE_SPECULAR
uniform vec4  uLightSpecular; //IACO    [MAX_LIGHTS];
#endif
#ifdef USE_LIGHT_CUTOFF
uniform vec3  uLightDirection; //IACO   [MAX_LIGHTS];
...
uniform mat4 uTextureMatrix; //IACO [MAX_TEXTURE_UNITS];
...
varying vec4 varTexCoord; //IACO[MAX_TEXTURE_UNITS];
...
void main(void)
{
	...
	if( uUseTexture)
	{
		if(uRenderMode == SphereMap || uRenderMode == Reflection2Layer)
		{
			...
			varTexCoord = vec4((reflection.x / m + 0.5), (reflection.y / m + 0.5), 0.0, 0.0); 
		}
		else
		{
			varTexCoord = inTexCoord0;
			if(uUseTexMatrix)
				varTexCoord *= uTextureMatrix;
		}
	}
	/*if(uUseTexture[1])
	{
