...
bool CIrrRocketEventReceiver::OnEvent(const irr::SEvent& event)
{
    if (event.EventType == irr::EET_MOUSE_INPUT_EVENT)
    {
        ...
        {			
			if(GGameApp::getInstance()->getRocketGUI()->GetSkipGUIEvents()) 
				GGameApp::getInstance()->getEventManager()->OnMouseInputEvent(event);
			else
    ...
    else if (event.EventType  == irr::EET_KEY_INPUT_EVENT)
    {
		...
		GGameApp::getInstance()->getEventManager()->OnKeyInputEvent(event);
