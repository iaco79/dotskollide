...
IrrRocketRenderer::IrrRocketRenderer(irr::video::IVideoDriver* driver) : Driver(driver)
{
	...
	//mColorToStatic=mColorTo;
	//mColorFromStatic=mColorFrom;
		

}

void IrrRocketRenderer::Update()
{
...
void IrrRocketRenderer::RenderGeometry(Rocket::Core::Vertex* vertices, int num_vertices, int* indices, int num_indices, Rocket::Core::TextureHandle texture, const Rocket::Core::Vector2f& translation, bool isText)
{
...
void IrrRocketRenderer::RenderCompiledGeometry(Rocket::Core::CompiledGeometryHandle geometry, const Rocket::Core::Vector2f& translation, bool isText)
{
    //irr::core::vector3df scaleSet(1024.f/Driver->getViewPort().getWidth(), 768.f/Driver->getViewPort().getHeight(), 1);

    irr::scene::SMeshBuffer* buffer = (irr::scene::SMeshBuffer*)geometry;
	float fColorFactor=0.0f;
	...
	if(!isText)
	{
		for (irr::u32 i=0;i<buffer->Vertices.size();i++)
		{
			...
			irr::video::S3DVertex&  vx = buffer->Vertices[i];
			fColorFactor= (vx.Pos.Y + translation.y) / (float)Driver->getViewPort().getHeight();

			
			color = mColorTo.getInterpolated(mColorFrom,fColorFactor);
			vx.Color = color;
		
			//.push_back(irr::video::S3DVertex(vertices[i].position.x, vertices[i].position.y, 0, 0, 0, 1, irr::video::SColor(vertices[i].colour.alpha, vertices[i].colour.red, vertices[i].colour.green, vertices[i].colour.blue), vertices[i].tex_coord.x, vertices[i].tex_coord.y));
		}
	}
