...
static int sdl_mix_musiclength=0;

static int sdl_mix_musicwatchdog=0;
...
{       
		...
		SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,"music_ogg start OGG_DecodeThread\n");
        ...
        while(sharedPlaying)
        {
			...
			if(threadAlive>100)
			{
				SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,"music_ogg threadAlive\n");
		...
		SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,"music_ogg finished OGG_DecodeThread\n");
...
void OGG_play(OGG_music *music)
{
	    if(music->playing == 0)
		{
			...
			SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,"music_ogg OGG_play\n");
		...
		#else
		#ifdef OGG_USE_TREMOR
			sdl_mix_musiclength = (int)ov_time_total(&music->vf, -1);

			SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,"music_ogg len:%d", (int)sdl_mix_musiclength);
...
/* Read some Ogg stream data and convert it for output */
	...
	static void OGG_getsome(void  *msc)
	{
		...
		int offset = (int)ov_time_tell(&music->vf);
		
		sdl_mix_musicwatchdog++;

		if(sdl_mix_musicwatchdog>100)
		{
			SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,"music_ogg offset is :%d", (int)offset);
			SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,"music_ogg len is :%d", (int)len);
			
			sdl_mix_musicwatchdog=0;
		}


		if((offset+len)>sdl_mix_musiclength)
		{
			
			SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,"music_ogg finish with offset :%d", (int)offset);
		...
		if ( len <= 0 ) {
			...
			SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,"music_ogg len for return is :%d", (int)len);
