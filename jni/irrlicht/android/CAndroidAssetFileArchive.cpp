...
{
	os::Printer::log("Requesting file:");
	os::Printer::log(filename.c_str());

    CAndroidAssetReader *reader = new CAndroidAssetReader(AssetManager, filename);

    if(reader->isOpen())
	{
		 os::Printer::log("Requested file opened.");
...
{
	os::Printer::log("Listing APK file");
	...
	while(const char *filename = AAssetDir_getNextFileName(dir))
	{
		...
		addItem(full_filename, /*offet*/0, /*size*/0, /*isDir*/false, getFileCount());
		 os::Printer::log(full_filename.c_str());
	}
	AAssetDir_close(dir);
}
