	...
	)
{
	...
	//IACO setup ZIP file
	InitializeFileSystemExtern(FileSystem);
...
void  CIrrDeviceStub::GetMoveSpeed(float& speedX, float& speedY)
{
	speedX = moveSpeedX;
	speedY = moveSpeedY;
...
void CIrrDeviceStub::dispatchEventsFromUser(void)
{	
	if(eventQueue.size()>0)
	{
		SDL_mutexP(queuelock);

		for(int i=0;i<eventQueue.size();i++)
		{
			postEventFromUser(eventQueue[i]);
		}
		eventQueue.clear();

		SDL_mutexV(queuelock);
...
void CIrrDeviceStub::enqueueEventsFromUser(const SEvent& event)
{
	SDL_mutexP(queuelock);
	if(event.EventType  == irr::EET_MOUSE_INPUT_EVENT)
	{
		...
		{
			moveSpeedX = event.MouseInput.SX;
			moveSpeedY = event.MouseInput.SY;
