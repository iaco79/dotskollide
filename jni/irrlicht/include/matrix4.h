	...
	class CMatrix4
	{
		public:
			...
			*/
			...
			CMatrix4<T>& buildTextureTransRotateCenter(f32 rotateRad,
					const core::vector2df &rotatecenter);
...
//		M[14] = (T)(2.0f*zNear*zFar/(zNear-zFar)); // OpenGL version
	...
	*/
	...
	template <class T>
	inline CMatrix4<T>& CMatrix4<T>::buildTextureTransRotateCenter( f32 rotateRad,
			const core::vector2df &rotatecenter)
	{
		const f32 c = cosf(rotateRad);
		const f32 s = sinf(rotateRad);
		...
		M[8] = (T)(rotatecenter.X - r00*rotatecenter.X - r01*rotatecenter.Y);
		M[9] = (T)(rotatecenter.Y - r10*rotatecenter.X - r11*rotatecenter.Y);
		M[10] = 1;
		M[11] = 0;
