...
namespace irr
{
    namespace video
    {
		...
		bool COGLES2SLMaterialRenderer::setPixelShaderConstant( const c8* name, int value)
		{
			...
			for (i=0; i < num; i++)
			{
				...
				//	os::Printer::log( "Setting shader uniform",ELL_WARNING);
					//os::Printer::log( name,ELL_WARNING);
					setUniform(i,&value,1);
					return true;
				}	
			}
            return false;
        ...
        bool COGLES2SLMaterialRenderer::setUniform( int index, const void* data, int count )
        {
            ...
            switch ( ui.type )
            {
				...
				case GL_SAMPLER_2D:
					{
						//os::Printer::log( "Setting sampler 2d.",ELL_WARNING);

						int v  = (*(int*)data);
						glUniform1i(ui.location, v);
					}
					break;
                default:
