...
namespace irr
{
	...
	public:
		...
		virtual void dispatchEventsFromUser(void);
		virtual void enqueueEventsFromUser(const SEvent& event);
		...
		//warning this method shoud only be accessed inside an OnEvent handler (for thread safety)
		//retrieves the speedX and speedY when processing a MOUSE move event
		void GetMoveSpeed(float& speedX, float& speedY);
