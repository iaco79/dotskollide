	...
	if (FileSystemType == FILESYSTEM_NATIVE)
	{
		...
		if( (hFile = _findfirst( "*", &c_file )) != -1L )
		{
			do
			{
				...
				r->addItem(fullPath, 0,c_file.size, (_A_SUBDIR & c_file.attrib) != 0, 0);
			}
		...
		if (dirHandle)
		{
			...
			while ((dirEntry=readdir(dirHandle)))
			{
				...
				r->addItem(fullPath, 0,size, isDirectory, 0);
			}
			closedir(dirHandle);
		}
		#endif
		...
		for (u32 i=0; i < FileArchives.size(); ++i)
		{
			...
			for (u32 j=0; j < merge->getFileCount(); ++j)
			{
				if (core::isInSameDirectory(Path, merge->getFullFileName(j)) == 0)
				{
					...
					r->addItem(fullPath, merge->getFileOffset(j), merge->getFileSize(j), merge->isDirectory(j), 0);
				}
			}
		}
