	...
	const core::position2d<s32> & pos, ICameraSceneNode* camera)
{
	...
	__android_log_print(ANDROID_LOG_INFO, "Irrlicht", "LeftToRight %d, %.2f, %.2f, %.2f", 
		camera->getID(),lefttoright.X,lefttoright.Y,lefttoright.Z);
	__android_log_print(ANDROID_LOG_INFO, "Irrlicht", "LeftToDown %d, %.2f, %.2f, %.2f", 
		camera->getID(),uptodown.X,uptodown.Y,uptodown.Z);
